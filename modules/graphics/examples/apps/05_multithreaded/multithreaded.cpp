...
REGISTER_APP_INSTANCE(MultithreadedApp, multithreadedAppInstance)
App* getMultithreadedAppInstance() {
	return multithreadedAppInstance;
}

MultithreadedApp::MultithreadedApp() {
	// register app instance
	if (!registerApp(5, this)) {
		std::cerr << "[MultithreadedApp] ERROR: Failed to register app instance (index=4)." << std::endl;
	}
}
...
void MultithreadedApp::drawCube(LA::mat4 _model) {
	Encoder* encoder = begin();
	if (!encoder) {
		std::cout << "[MultithreadedApp] ERROR: Failed to get encoder in drawCube." << std::endl;
		return;
	}
...
void MultithreadedApp::drawTexTri(LA::mat4 _model, TextureHandle _hTex) {
	// open encoder
	Encoder* encoder = begin();
	if (!encoder) {
		std::cout << "[MultithreadedApp] ERROR: Failed to get encoder in drawQuad." << std::endl;
		return;
	}
...
void MultithreadedApp::draw(double _dt) {
	// get frame consts
	const Resolution& res = getResolution();
	...
	MultithreadedApp::drawCube(model);

	// draw fb/tex on triangle
	LA::mat4 mat = LA::mat4();
	
	// spawn threads to draw cube left/right
	LA::mat4 modelBottomLeft = LA::Translate(-0.5f, -0.5f, 0.0f) * LA::Scale(0.5f, 0.5f, 0.5f);
	LA::mat4 modelTopLeft = LA::Translate(-0.5f, 0.5f, 0.0f)* LA::Scale(0.5f, 0.5f, 0.5f);
	LA::mat4 modelBottomRight = LA::Translate(0.5f, -0.5f, 0.0f) * LA::Scale(0.5f, 0.5f, 0.5f);
	LA::mat4 modelTopRight = LA::Translate(0.5f, 0.5f, 0.0f) * LA::Scale(0.5f, 0.5f, 0.5f);
	std::thread t1(&MultithreadedApp::drawTexTri, this, modelBottomLeft, m_hFBColourTex);
	std::thread t2(&MultithreadedApp::drawTexTri, this, modelTopLeft, m_hFBColourTex);
	std::thread t3(&MultithreadedApp::drawTexTri, this, modelBottomRight, m_hFBColourTex);
	std::thread t4(&MultithreadedApp::drawTexTri, this, modelTopRight, m_hFBColourTex);

	// block until both draw threads complete
	t1.join();
	t2.join();
	t3.join();
	t4.join();
	...
	std::cout << "[MultithreadedApp] Completed Frame " << frameIdx << std::endl;
};

void MultithreadedApp::shutdown() {
	// destroy resources
	destroy(m_hTexture0);
